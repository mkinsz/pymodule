...
def read_frame_as_jpeg(in_file, out,frame_num,max_frame):
    ...
    while rval: #循环读取视频帧
        ...
        if rval:
            ...
            if c>=int(frame_num) and c< int(max_frame):
                #print(c,rval)
                #print(c)
...
if __name__ == '__main__':
    file_path = r"E:\\work\\相关数据\\标定数据\\"
    out=r"E:\\work\\相关数据\\标定数据"
    ...
    for root,dirs,files in os.walk(file_path):
        for file in files:
            if file.endswith(".ts"):
                ...
                extract_frame(file_path,950)
                # read_frame_as_jpeg(file_path, out, 1, 17000)
